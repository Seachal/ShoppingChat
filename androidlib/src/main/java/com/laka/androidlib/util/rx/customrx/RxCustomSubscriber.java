package com.laka.androidlib.util.rx.customrx;
    ...
    public void onError(Throwable e) {
        ...
        if (e instanceof BaseException) {
            ...
            if (exception.getCode() == RequestCommonCode.LK_WRONG_USER_TOKEN
                ...
                if (mIsToLogin) {
                    // 强制跳转到用户登陆页面，目前已经通过拦截器实现，无需在这里跳转
            ...
            if (mCallBack != null) {
                e.printStackTrace();
                LogUtils.error("网络不可用");
                mCallBack.onFail(new ApiException(RequestCommonCode.LK_ERROR_NETWORK, RequestCommonCode.LK_ERROR_NETWORK_MSG));
            }
        } else { // 异常错误
