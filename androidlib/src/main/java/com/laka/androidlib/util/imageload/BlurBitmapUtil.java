...
public class BlurBitmapUtil {
    ...
    private static BlurBitmapUtil sBlurBitmapUtil;

    public static BlurBitmapUtil getInstance() {
        if (sBlurBitmapUtil == null) {
            synchronized (BlurBitmapUtil.class) {
                if (sBlurBitmapUtil == null) {
                    sBlurBitmapUtil = new BlurBitmapUtil();
                }
            }
        }
        return sBlurBitmapUtil;
    }
     ...
     * @param outWidth  图片输出尺寸，尺寸越小，也会越模糊
     * @param outHeight 图片输出尺寸，尺寸越小，也会越模糊
     * @return 模糊处理后的Bitmap
     */
