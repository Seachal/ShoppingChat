...
public class NimUserInfoCache {
    ...
    /**
     * 从缓存中获取用户信息，如果获取不到，请通过 getUserInfoFromRemote 异步方法获取
     */
    public NimUserInfo getUserInfo(String account) {
        if (account2UserMap.isEmpty()) buildCache();
        ...
        NimUserInfo userInfo = account2UserMap.get(account);
        if (userInfo == null) {
            userInfo = NIMClient.getService(UserService.class).getUserInfo(account);
            if (userInfo != null) {
                account2UserMap.put(account, userInfo);
            }
        }
        if (userInfo == null) {
            //异步更新用户信息到缓存
