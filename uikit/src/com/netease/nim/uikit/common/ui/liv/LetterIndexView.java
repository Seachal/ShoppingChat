...
public class LetterIndexView extends View {
    ...
    public LetterIndexView(Context paramContext, AttributeSet paramAttributeSet, int paramInt) {
        ...
        this.offset = 0.0F;
        this.hit = false;
        this.normalColor = ContextCompat.getColor(getContext(), R.color.color_2c2c2c);
        this.bgaCircleColor = ContextCompat.getColor(getContext(), R.color.color_09C266);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        float textSize = letterHeight * 7 / 12;
        ...
        for (int i = 0; i < letters.length; ++i) {
            ...
            int cellNumber = (int) (offset / letterHeight);
            if (hit && i == cellNumber) {
                mPaint.setColor(bgaCircleColor);
                float dotX = width / 2;
                float startY = cellNumber * letterHeight;
                float endY = (cellNumber + 1) * letterHeight;
                float dotY = (startY + endY) / 2;
                LogUtils.log("onDrawable-----dotX=" + dotX + "-----dotY=" + dotY + "----i=" + i);
                canvas.drawCircle(dotX, dotY, letterHeight / 2, mPaint);
