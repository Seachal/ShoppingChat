...
import com.netease.nim.uikit.business.session.listener.CustomAudioControlListener;
...
public class MsgViewHolderAudio extends MsgViewHolderBase {
    ...
    protected void bindContentView() {
        onPlayListener.setImMessage(message);
    ...
    //创建一个独立的外部类，并且用软引用引用 MsgViewHolderAudio
    private CustomAudioControlListener onPlayListener = new CustomAudioControlListener(this);

//     audioControl 是单例，持有onPlayListener 的引用，匿名内部类隐式持有外部类的引用，导致内存泄漏
...
//    };
    ...
    private void endPlayAnim() {
        if (isReceivedMessage()) {
            animationView.setBackgroundResource(R.drawable.chat_icon_video_l_03);
        } else {
            animationView.setBackgroundResource(R.drawable.chat_icon_video_r_03);
        }
    }

    public boolean isTheSame(String uuid) {
