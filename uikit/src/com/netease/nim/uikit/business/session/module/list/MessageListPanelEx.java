...
import com.netease.nim.uikit.business.forward.ForwardActivity;
 ...
 * @Author:summer
 * @Date:2019/9/2
 * @Description: 基于 RecyclerView 的消息收发模块
 */
public class MessageListPanelEx {
    ...
    private static final int REQUEST_CODE_FORWARD = 0x03;
    ...
    //presenter
    private AvatarLongClickedListener avatarLongClickedListener;
    ...
    private ReceiveRedPackageHelper receiveRedPackageHelper;

    public void onCreate() {
        if (receiveRedPackageHelper == null) {
            receiveRedPackageHelper = new ReceiveRedPackageHelper(container.activity);
            receiveRedPackageHelper.setRefreshListListener(new Function0<Unit>() {
                @Override
                public Unit invoke() {
                    refreshMessageList();
                    return null;
    ...
    public void onDestroy() {
        ...
        if (receiveRedPackageHelper != null) {
            receiveRedPackageHelper.onDestory();
        }
    }
...
//            // resend的的情况，可能时间已经变化了，这里要重新检查是否要显示时间
    ...
    private class MessageLoader implements BaseFetchLoadAdapter.RequestLoadMoreListener, BaseFetchLoadAdapter.RequestFetchMoreListener {
        ...
        private void onMessageLoaded(final List<IMMessage> messages) {
            ...
            //不满 loadMsgCount 条数据，直接不会触发加载更多
            ...
            while (iterator.hasNext()) {
                ...
                if (imMessage.getAttachment() instanceof RobRedPackageAttachment
                        && !(imMessage.getAttachment() instanceof RedPackageAttachment)) {
                    RobRedPackageAttachment attachment = (RobRedPackageAttachment) imMessage.getAttachment();
                    if (!NimUIKitImpl.getAccount().equals(attachment.getmOpUser())
                            && !NimUIKitImpl.getAccount().equals(attachment.getmUserId())) {
        ...
        private void onAnchorContextMessageLoaded(final List<IMMessage> messages) {
            ...
            boolean noMoreData = messages.size() < loadMsgCount;
...
//                break;
//        }
        final ArrayList<String> selected = data.getStringArrayListExtra(ForwardActivity.RESULT_DATA);
        if (CommonUtil.isEmpty(selected)) {
            return;
        }
        switch (requestCode) {
            case REQUEST_CODE_FORWARD:
                for (String contactId : selected) {
                    if (MultiContactItem.isP2P(contactId)) {
                        doForwardMessage(MultiContactItem.decodeContactId(contactId), SessionTypeEnum.P2P);
                    } else {
                        doForwardMessage(MultiContactItem.decodeContactId(contactId), SessionTypeEnum.Team);
                    }
                }
                break;
        }
    ...
    private class MsgItemEventListener implements MsgAdapter.ViewHolderEventListener {
        ...
        //双击
        @Override
        public void onItemDoubleClick(View itemView, IMMessage message) {
            //if (message.getMsgType() == MsgTypeEnum.text) { //文本消息
            //KeyboardHelper.hideKeyBoard(container.activity, messageListView);
            //new TextMessageDialog(container.activity).show();
            //}
        }

        @Override
        public void onItemClick(View itemView, IMMessage message) {
            if (message.getAttachment() instanceof RedPackageAttachment) {//红包消息
                onCreate();
                receiveRedPackageHelper.onReceiveRedPackageStart(message);
            } else if (message.getAttachment() instanceof RobRedPackageAttachment) { //抢红包通知，直接进入领取红包页面
                onCreate();
                receiveRedPackageHelper.onRedPackageTicketStart(message);
            }
        }
        ...
        @Override
        public void onAvatarLongClicked(Context context, View clickView, IMMessage message) {
            if (avatarLongClickedListener != null) {
                avatarLongClickedListener.onAvatarLongClicked(clickView, message);
            }
        }
        ...
        private void prepareDialogItems(final IMMessage selectedItem, CustomAlertDialog alertDialog) {
            ...
            // 3 revoke 测回
            if (selectedItem.getFromAccount().equals(NimUIKitImpl.getAccount())) {
            ...
            if (!NimUIKitImpl.getMsgForwardFilter().shouldIgnore(selectedItem) && !recordOnly) {
                longClickItemForward(selectedItem, alertDialog);
//                // 6 forward to person 转发到个人
//                longClickItemForwardToPerson(selectedItem, alertDialog);
//                // 7 forward to team 转发到群
//                longClickItemForwardToTeam(selectedItem, alertDialog);
            }
            // 7 cancel upload attachment 取消上传附件，暂且隐藏
            //longClickItemCancelUpload(selectedItem, alertDialog);
            //设置弹窗动画
            alertDialog.getWindow().setWindowAnimations(R.style.updateAnimation);
        }
        ...
        // 长按菜单项 -- 转发
        private void longClickItemForward(final IMMessage item, CustomAlertDialog alertDialog) {
            alertDialog.addItem(container.activity.getString(R.string.forward), new CustomAlertDialog.onSeparateItemClickListener() {

                @Override
                public void onClick() {
                    forwardMessage = item;
                    ForwardActivity.startActivityForResult(container.activity, item.getContent(), REQUEST_CODE_FORWARD);
                }
            });
        }
...
//                    Intent intent = new Intent(container.activity, ForwardActivity.class);
//                    intent.putExtra(ForwardActivity.FORWARD, item.getContent());
//                    container.activity.startActivity(intent);
                    ForwardActivity.startActivityForResult(container.activity, item.getContent(), REQUEST_CODE_FORWARD);
                }
            });
            ...
            NIMClient.getService(MsgService.class).sendMessage(message, false)
                    .setCallback(new RequestCallback<Void>() {
                        ...
                        public void onSuccess(Void aVoid) {
                            ToastHelper.showToast(container.activity, "转发成功");
                        }

                        @Override
                        public void onFailed(int i) {
                            ToastHelper.showToast(container.activity, "转发失败");
                        }

                        @Override
                        public void onException(Throwable throwable) {
                            ToastHelper.showToast(container.activity, "转发异常");
                        }
                    });
    ...
    public void setAvatarLongClickedListener(AvatarLongClickedListener avatarLongClickedListener) {
        this.avatarLongClickedListener = avatarLongClickedListener;
    }

    public interface AvatarLongClickedListener {
