...
public abstract class MsgViewHolderBase extends RecyclerViewHolder<BaseMultiItemFetchLoadAdapter, BaseViewHolder, IMMessage> {
    ...
    // contentContainerView的默认长按事件。如果子类需要不同的处理，可覆盖 onItemLongClick 方法
    ...
    // 内容区域点击事件响应处理。
    protected void onItemDoubleClick(View view) {
    }
    ...
    protected boolean shouldDisplayNick() {
        return message.getSessionType() == SessionTypeEnum.Team && isReceivedMessage() && !isMiddleItem() && getMsgAdapter().needShowName();
    }
        ...
        if (NimUIKitImpl.getSessionListener() != null) {
            View.OnLongClickListener longClickListener = new View.OnLongClickListener() {
                ...
                public boolean onLongClick(View v) {
                    ...
                    if (getMsgAdapter().getEventListener() != null) {
                        getMsgAdapter().getEventListener().onAvatarLongClicked(context, avatarLeft, message);
                        return true;
                    }
