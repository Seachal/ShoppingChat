...
public class ContactSearch {
    ...
    /**
     * 判断是否选中联系人（通过关键词匹配name 和 account）
     */
    static boolean hitUser(UserInfo contact, TextQuery query) {
        ...
        return TextSearcher.contains(query.t9, name, query.text) /*|| TextSearcher.contains(query.t9, account, query.text)//联系人模糊查找不匹配 account*/;
    }

    /**
     * 判断是否选中联系人（通过关键词匹配 alias 和 account ）
     */
    static boolean hitFriend(UserInfo contact, TextQuery query) {
        String account = contact.getAccount();
        String alias = NimUIKit.getContactProvider().getAlias(account);
        return TextSearcher.contains(query.t9, alias, query.text) /*TextSearcher.contains(query.t9, account, query.text) || //联系人模糊查找不匹配 account*/;
    }

    /**
     * 判断是否选中群（只匹配群名称，不匹配群ID）
     */
