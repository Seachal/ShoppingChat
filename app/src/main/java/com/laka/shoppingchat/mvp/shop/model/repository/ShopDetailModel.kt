...
import com.laka.shoppingchat.mvp.user.model.responsitory.UserCustomRetrofitHelper
...
class ShopDetailModel : IShopDetailContract.IShopDetailModel {
    ...
    override fun onGetCouponInfo(params: MutableMap<String, Any>, callBack: ResponseCallBack<CouponInfo>) {
        ShopDetailCustomRetrofitHelper.instance
            .onCouponInfo(params)
            .compose(RxSchedulerComposer.normalSchedulersTransformer())
            .subscribe(object : RxCustomSubscriber<CouponInfo, IShopDetailContract.IShopDetailView>(mView, callBack) {
    ...
    override fun onLoadProductDetail(params: HashMap<String, String>, callBack: ResponseCallBack<CustomProductDetail>) {
        ShopDetailCustomRetrofitHelper.instance
            ...
            .compose(RxSchedulerComposer.normalSchedulersTransformer())
            .subscribe(object :
                RxCustomSubscriber<CustomProductDetail, IShopDetailContract.IShopDetailView>(mView, callBack) {
                override fun onSubscribe(d: Disposable) {
                    super.onSubscribe(d)
                    mDisposableList.add(d)
                }
     ...
     * 加载相关推荐商品列表
     * */
    override fun onLoadRecommendData(
        params: HashMap<String, String>,
        callBack: ResponseCallBack<ProductRecommendResponse>
    ) {
        ShopDetailCustomRetrofitHelper.instance
            .onLoadRecommendData(params)
            .compose(RxSchedulerComposer.normalSchedulersTransformer())
            .subscribe(object :
                RxCustomSubscriber<ProductRecommendResponse, IShopDetailContract.IShopDetailView>(mView, callBack) {
                override fun onSubscribe(d: Disposable) {
                    super.onSubscribe(d)
                    mDisposableList.add(d)
                }
     ...
     * 获取产品详情服务器地址
     * */
    override fun onGetProductDetailH5ServiceUrl(
        params: HashMap<String, String>,
        ...
        UserCustomRetrofitHelper.instance
            ...
            .compose(RxSchedulerComposer.normalSchedulersTransformer())
            .subscribe(object :
                RxCustomSubscriber<HashMap<String, String>, IShopDetailContract.IShopDetailView>(mView, callBack) {
                override fun onSubscribe(d: Disposable) {
                    super.onSubscribe(d)
                    mDisposableList.add(d)
                }
        ...
        call.enqueue(object : Callback {
            ...
            override fun onResponse(call: Call?, response: Response?) {
                ...
                if (code == 200) {
                    ...
                    ApplicationUtils.getMainHandler().post {
                        callBack.onFail(object :
                            BaseException(RequestCommonCode.LK_ERROR_REDIS_CONNECT_FATLED, "请求失败") {})
                    }
                }
            }
        })
                                        ...
                                        if (itemObject.has("videos")) {
                                            ...
                                            if (videoArray != null && videoArray.length() > 0) {
                                                ...
                                                productVideo.videoUrl =
                                                    if (videoObject.getString("url").startsWith("http")) {
                                                        "${videoObject.getString("url")}"
                                                    } else {
                                                        "http:${videoObject.getString("url")}"
                                                    }
                                                productVideo.videoThumbnailURL =
                                                    "${videoObject.getString("videoThumbnailURL")}"
                                            }
                                        }
                                    }
                                }
                    ...
                    if (dataObject.has("seller")) {
                        ...
                        for (i in 0 until evaluatesArray.length()) {
                            ...
                            try {
                                ...
                                targetEvaluate.levelBackgroundColor =
                                    "${evaluatesObject.getString("levelBackgroundColor")}"
                            } catch (e: Exception) {
                                e.printStackTrace()
                            }
