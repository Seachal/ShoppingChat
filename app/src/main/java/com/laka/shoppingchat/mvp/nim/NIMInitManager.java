...
import com.laka.shoppingchat.mvp.nim.event.OnlineStateEventManager;
...
public class NIMInitManager {
    ...
    private void registerLocaleReceiver(boolean register) {
        if (register) {
            ...
            ApplicationUtils.getContext().registerReceiver(localeReceiver, filter);
        } else {
            ApplicationUtils.getContext().unregisterReceiver(localeReceiver);
        }
    }
    ...
    private void registerBroadcastMessages(boolean register) {
        NIMClient.getService(MsgServiceObserve.class).observeBroadcastMessage(new Observer<BroadcastMessage>() {
            ...
            public void onEvent(BroadcastMessage broadcastMessage) {
                ToastHelper.showToast(ApplicationUtils.getContext(), "收到全员广播 ：" + broadcastMessage.getContent());
            }
        }, register);
    }
