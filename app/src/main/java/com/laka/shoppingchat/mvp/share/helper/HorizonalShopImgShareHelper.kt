package com.laka.shoppingchat.mvp.share.helper
...
import com.laka.shoppingchat.common.threadpool.ThreadPoolHelp
import com.laka.shoppingchat.common.threadpool.ThreadPoolType
    ...
    fun initViewAndData(
        ...
        for (i in 0 until mImageList.size) {
            val itemView = LayoutInflater.from(context)
                .inflate(R.layout.item_wechat_share_shop_img, mLlRootView, false)
            ...
            val layoutParams =
                LinearLayout.LayoutParams(ScreenUtils.dp2px(85f), ScreenUtils.dp2px(85f))
            ...
            GlideLoader.loadImage(mContext, mImageList[i], ivImg)
            if (i == 0) {
                mCheckArray[0] = 1
...
//            ShopShareModuleNavigator.startSeeBigImageActivity(mContext, list, tag)
        ...
        for (i in 0 until mCheckArray.size) {
            if (mCheckArray[i] == 1) {
                if (i != mExtensionPosition) { //推广图更换了
                    val preTxtView = mLlRootView.getChildAt(mExtensionPosition)
                        .findViewById<TextView>(R.id.tv_txt)
    ...
    private fun saveBitmap(): Boolean {
        ...
        return BitmapUtils.gainBitmapFromByte(mScreenshotData[mExtensionPosition])
            .compress(Bitmap.CompressFormat.JPEG, 60, FileOutputStream(mSaveFile))
    }
    ...
    private fun saveNormalImageTask(
        resource: Bitmap?,
        file: File,
        saveSuccessListener: (pos: Int, rootFile: File, fileList: ArrayList<File>) -> Unit,
        folder: File,
    ...
    private fun saveExtensionImageTask(
        file: File, saveFailListener: () -> Unit,
        saveSuccessListener: (pos: Int, rootFile: File, fileList: ArrayList<File>) -> Unit,
    ...
    //保存相册成功
    fun saveAllImageSuccess(
        finishListener: (pos: Int, rootFile: File, fileList: ArrayList<File>) -> Unit,
        rootFile: File,
        fileList: ArrayList<File>
    ) {
    ...
    fun setOnCreateScreenShotStatusListener(
        startListener: ((pos: Int) -> Unit),
        finishListener: ((pos: Int) -> Unit)
    ) {
