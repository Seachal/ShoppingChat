package com.laka.shoppingchat.mvp.shop.utils
    ...
    companion object {
        ...
        fun add(vararg args: String): BigDecimal {
            return try {
                ...
                for (arg in args) {
                    var bigArg: BigDecimal =
                        if (TextUtils.isEmpty(arg) || arg == "null" || arg == "NULL") {
                            BigDecimal("0")
                        } else {
                            BigDecimal(arg)
                        }
        ...
        private fun roundForHalf(arg: BigDecimal, mode: RoundingMode): String {
            return arg?.setScale(mRoundDigit, mode).toString()
        }
