...
import com.laka.shoppingchat.mvp.nim.main.adapter.SystemMessageAdapter;
...
import com.laka.shoppingchat.mvp.nim.main.viewholder.SystemMessageViewHolder;
...
public class SystemMessageActivity extends UI implements TAdapterDelegate,
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        TitleBarView titleBar = findViewById(R.id.title_bar);
        titleBar.setLeftIcon(R.drawable.selector_nav_btn_back)
                ...
                .setOnRightClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        AddFriendsActivity.Companion.start(SystemMessageActivity.this);
                    }
                });
        initAdapter();
        initListView();
        SPHelper.putInt(UserUtils.getImAccount(), 0);
...
//    }
    ...
    public void loadMessages() {
        ...
        while (true) {
            ...
            for (SystemMessage m : temps) {
                ...
                //过滤掉消息非加好友    attachData.getEvent() == AddFriendNotify.Event.RECV_ADD_FRIEND_VERIFY_REQUEST
                if (MessageHelper.filterMessage(m)) {
                    continue;
                }
                // 保存有效消息
                items.add(m);
    ...
    private Boolean filterMessage(SystemMessage message) {
...
//                    sb.append("请求添加好友" + (TextUtils.isEmpty(message.getContent()) ? "" : "：" + message.getContent()));
    ...
    private void onIncomingMessage(final SystemMessage message) {
        AddFriendNotify attachData0 = null;
        Object attachObject = message.getAttachObject();
        if (attachObject instanceof AddFriendNotify) {
            attachData0 = (AddFriendNotify) attachObject;
        }
        if (message.getType() != SystemMessageType.AddFriend) {
            return;
        } else {
            if (attachData0 != null && attachData0.getEvent() != AddFriendNotify.Event.RECV_ADD_FRIEND_VERIFY_REQUEST) {
                return;
            }
        }
        // 同一个账号的好友申请仅保留最近一条
    ...
    private void onProcessSuccess(final boolean pass, SystemMessage message) {
        ...
        IMMessage msg = MessageBuilder.createTextMessage(message.getFromAccount(), SessionTypeEnum.P2P,
                message.getContent());
        msg.setDirect(MsgDirectionEnum.In);
        msg.setStatus(MsgStatusEnum.success);
        msg.setFromAccount(message.getFromAccount());

        if (pass && message.getType() == SystemMessageType.ApplyJoinTeam) {
            List<String> strings = TeamCreateHelper.requestMembers(message.getTargetId());
            strings.add(message.getFromAccount());
            TeamCreateHelper.createAvater(this, message.getTargetId(), strings);
        } else if (pass && message.getType() == SystemMessageType.RejectTeamApply) {

        } else {
            // 发送给对方
            IMMessage msg1 = MessageBuilder.createTextMessage(
                    message.getFromAccount(), SessionTypeEnum.P2P,
                    "我通过了你的朋友验证请求，现在我们可以开始聊天了"
            );
            NIMClient.getService(MsgService.class).sendMessage(msg1, false);
    ...
    public void onEvent(AgreeEvent event) {
        for (SystemMessage message : items) {
            if (message.getFromAccount().equals(event.account)) {
                if (event.agree) {
                    onAgree(message);
                } else {
                    onReject(message);
