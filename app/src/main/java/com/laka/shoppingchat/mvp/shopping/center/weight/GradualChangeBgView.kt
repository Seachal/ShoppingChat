...
import com.laka.shoppingchat.common.util.color.ColorUtils
...
class GradualChangeBgView : View {
    ...
    private var mInitialRightHeight = ScreenUtils.dp2px(100f).toFloat() //右下角初始高度
    //左下角轨迹的点
    private var mLeftBottomX = 0f
    private var mLeftBottomY = 0f
    private var mInitialLeftHeight = ScreenUtils.dp2px(50f).toFloat() //左下角初始高度
    //绘制的内容区的固定高度，当配合列表滑动时，底部圆弧会做形变动画，到mLeftBottomY=0,
    //添加一个 mQuadFixedHeight 高度控制后，底部圆弧形变动画只能做到 mLeftBottomY = mQuadDixedHeight
    private var mQuadFixedHeight = ScreenUtils.getStatusBarHeight()
    ...
    fun setScrollerData(verticalOffest: Int) {
        ...
        mQuadY = mRightBottomY
        mLeftBottomY =
            mInitialLeftHeight + (verticalOffest * (mLeftBottomSpotMoveDistance / mRightBottomSpotMoveDistance.toFloat()))
        //越界处理
    ...
    fun deformForRefreshLayout(verticalY: Int) {
        ...
        var nextLeftY = mInitialLeftHeight + (verticalY * 3 / 2)
        var nextRightY = mInitialRightHeight + verticalY
...
//        invalidate()
    ...
    /**根据滑动的距离计算是否到达顶部*/
    fun calculIsTop(verticalY: Int): Boolean {
        val leftBottomY =
            mInitialLeftHeight + (verticalY * (mLeftBottomSpotMoveDistance / mRightBottomSpotMoveDistance.toFloat()))
        val rightBottomY = (mInitialRightHeight + verticalY)
        if (leftBottomY <= mQuadFixedHeight || rightBottomY <= mQuadFixedHeight) {
            return true
        }
        return false
    }
