...
import com.laka.shoppingchat.mvp.shopping.center.model.bean.CategoryBean
import com.laka.shoppingchat.mvp.shopping.center.model.bean.ProductWithCoupon
import com.laka.shoppingchat.mvp.shopping.center.presenter.ShoppingListPresenter
import com.laka.shoppingchat.mvp.shopping.center.view.adapter.ProductListAdapter
 ...
 */
class ShoppingListFragment : BaseProductListFragment<ProductWithCoupon>(),
    IShoppingListContract.IShoppingView,
    ...
    override fun initView(rootView: View?, savedInstanceState: Bundle?) {
        ...
        if (mStaggeredGridItemDecoration == null) {
            mStaggeredGridItemDecoration = SpacesStaggeredDecoration(object :
                SpacesStaggeredDecoration.ItemDecorationCallBack {
        ...
        mRvList.enableRefresh(true)
        mRvList.setLoadingView(
    ...
    override fun initDataLazy() {
        if (!isNetWorkError) {
            mRvList.refresh()//不做动画的刷新
            mRvList.showLoadingView()
        }
        mRvList.onItemClickListener =
            object : RefreshRecycleView.OnItemClickListener<ProductWithCoupon> {
                override fun onItemClick(data: ProductWithCoupon?, position: Int) {
                    data?.let {
                        ShopDetailModuleNavigator.startShopDetailActivity(
                            activity!!,
                            "${data.productId}"
                        )
                    }
    ...
    override fun initEvent() {
        mRvList.onScrollListener =
            RefreshRecycleView.OnScrollListener { recyclerView, _, dy, _, _ ->
                mTotalDy += dy
                ...
                EventBusManager.postEvent(
                    Event(
                        HomeEventConstant.EVENT_RECYCLER_VIEW_SCROLL,
