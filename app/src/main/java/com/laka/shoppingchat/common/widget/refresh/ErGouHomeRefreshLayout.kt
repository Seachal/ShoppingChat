package com.laka.shoppingchat.common.widget.refresh
...
import com.laka.shoppingchat.mvp.shopping.center.weight.GradualChangeBgView
import kotlin.math.abs

/**
 * @Author:summer
 * @Date:2019/5/9
 * @Description:购聊首页定制刷新布局
 */
class ErGouHomeRefreshLayout : LinearLayout {
    ...
    companion object {
        ...
        //logo 缩放动画的最大值
        const val SCALE_MAX_FOR_LOG0 = 2.5f
    }
    ...
    //触发刷新的距离
    private var mDistanceRefresh = ScreenUtils.dp2px(50f)
    ...
    private fun initView() {
        mHeaderView =
            LayoutInflater.from(context).inflate(R.layout.home_header_refresh_layout, this, false)
        mIvLoading = mHeaderView.findViewById(R.id.iv_loading)
        mIvLoading.setImageResource(R.drawable.anim_ergou_pull_refresh)
        mIvLoading.layoutParams.height = ScreenUtils.getStatusBarHeight()
        mIvLoading.layoutParams.width = ScreenUtils.getStatusBarHeight() * 2
    ...
    override fun onTouchEvent(event: MotionEvent?): Boolean {
        ...
        when (event?.actionMasked) {
            ...
            MotionEvent.ACTION_DOWN -> {//第一个手指按下
                ...
                mCurrentPointId = event.getPointerId(event.actionIndex)
                mLastPoint.set(
                    event.getX(event.findPointerIndex(mCurrentPointId)),
                    event.getY(event.findPointerIndex(mCurrentPointId))
                )
            ...
            MotionEvent.ACTION_UP -> {
                ...
                LogUtils.info(
                    "ergouRefreshLayout---onTouchEvent------Math.abs(scrollY)=${Math.abs(
                        scrollY
                    )}--------mMaxHandleDistance=$mMaxHandleDistance"
                )
                mIsNeedRefresh =
                    Math.abs(scrollY) > mDistanceRefresh && ::mRefreshListener.isInitialized
                handleReleaseAnim()
                mCurrentPointId = mInvalidPointId
            }
        }
    ...
    private fun handleActionPointerUp(event: MotionEvent) {
        ...
        mCurrentPointId =
            if (skipIndex == count - 1 && (skipIndex - 1) >= 0) { //最后一个手指抬起，则活动手指变为其前一个
    ...
    private fun handleHeaderViewTransparent(scrollY: Int) {
        val verticalScrollY = abs(scrollY)
        ...
        // refresh logo 缩放动画
        var temp = transpire * 2.0f
        val scale =
            if ((SCALE_RETA_FOR_LOGO + temp) > SCALE_MAX_FOR_LOG0) SCALE_MAX_FOR_LOG0 else (SCALE_RETA_FOR_LOGO + temp)
        ...
        layoutParams.height =
            (scale * mProgressBarHeight + ScreenUtils.getStatusBarHeight() * transpire).toInt()
        mIvLoading.layoutParams = layoutParams
        //设置ivLoading的paddingTop
        mIvLoading.setPadding(0, (ScreenUtils.getStatusBarHeight() * transpire).toInt(), 0, 0)
    }
